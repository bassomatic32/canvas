...
require.define("/client.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  ...
  window.onload = function() {
    var canvas, createHeight, ctx, heightMap, i, _i, _j;
    canvas = document.getElementById('canvas');
    ctx = canvas.getContext('2d');
    heightMap = [];
    for (i = _i = 0; _i <= 800; i = ++_i) {
      heightMap.push(-1);
    }
    heightMap[0] = Math.random() * 500;
    heightMap[800 - 1] = Math.random() * 500;
    createHeight = function(left, right, range) {
      var factor, mid, midHeight;
      if (left >= right) {
        return;
      }
      ...
      return createHeight(mid, right, range);
    };
    createHeight(0, 799, 400);
    ctx.beginPath();
    ctx.moveTo(0, 500 - heightMap[0]);
    for (i = _j = 0; _j <= 800; i = _j += 32) {
